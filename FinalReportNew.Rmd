...
wls_model = lm(logsal ~ Age + pts_adj + Multiteam + dws_adj + Pos_cat, data=data, weights=weights)
...
p1 <- ols_resid_with_weights
p2 <- wls_resids_with_weights
...
There's an outlying case with a large leverage and Cook's Distance. We'll use a t-test for a mean shift to formally test if this is an outlier.

```{r p7, echo = FALSE, warning = FALSE, message=FALSE, results = 'hide'}
...
test_outlier_meanshift(modB, which.max(cooks.distance(modB)))
